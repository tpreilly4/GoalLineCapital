...
Goal Line Capital is a financial tools iOS application built with SwiftUI and SwiftData. The app provides various calculators and expense tracking functionality for Goal Line Capital's clients and prospects.
...
- `Views/Components/DollarAmountTextField.swift` - Reusable currency input component
...
- Use `Double` type and `.currency(code: "USD")` formatting for financial calculations
